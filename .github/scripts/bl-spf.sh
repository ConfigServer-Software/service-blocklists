...
#   @path               .github/scripts/bl-block.sh
#   @args               bl-spf.sh <fileSaveto> <fileFallbackBlock>
#   @commands           1.  echo "yandex.ru" | CFG_SKIP_CIDR_DEDUPE=true CFG_SKIP_BOGON_FILTER=true .github/scripts/bl-spf.sh blocklists/privacy/yandex.ipset
#                       2.  echo "yandex.ru" | CFG_SKIP_CIDR_DEDUPE=true CFG_SKIP_BOGON_FILTER=true .github/scripts/bl-spf.sh blocklists/privacy/yandex.ipset yandex
#                       3.  echo "yandex.ru" | CFG_STDOUT=true CFG_SKIP_CIDR_DEDUPE=true CFG_SKIP_BOGON_FILTER=true .github/scripts/bl-spf.sh blocklists/privacy/yandex.ipset
#   @structure          📁 .github
#                           📁 scripts
#                               📄 bl-spf.sh
#                           📁 templates
...
app_name="Blocklist › SPF"                                                      # name of app
...
# #
#   Parse › SPF Record IP Extraction
#   
#   Parse the SPF record for a given domain, extract mechanisms (a, mx, ip4, 
#   ip6, include, redirect), resolves referenced hosts, recursively process
#   nested SPF includes/redirects, and outputs any valid IP addresses found.
#   
#   Run             dig +short txt google.com
#   Extract         v=spf1 include:_spf.google.com ~all
#                       OR
#                   v=spf1 ip4:203.0.113.0/24 ip4:198.51.100.0/24 include:_netblocks.google.com
#   Check for       include:_spf.google.com
#   Call            parse_spf_record "_spf.google.com"
#   
...
parse_spf_record()
{
    ...
    if [[ fqdn_i -lt 64 ]]; then
        dns_record=( $(dig +short txt "${fqdn}" | tr -d '"' | grep '^v=spf') )

        [ ${#dns_record[@]} -eq 0 ] && {
            error "    ⭕ No SPF record found for ${bluel}${fqdn}${greym}" >&2
            return
        }

        info "    📕 Fetching DNS record ${bluel}${fqdn_i}${greym} => ${bluel}${dns_record}${greym}" >&2
        ...
        for entry in ${dns_record[*]}; do
            case ${entry} in
                ...
                mx:* )
                    parse_mx_record "${entry#*:}" >/dev/null 2>&1
                    ;;
...
#   Parse › MX Record
#   
#   Parse MX records for a given domain, extract the mail server hostnames, 
#   and performs DNS lookups on each one (likely to trigger resolution or warm/cache results).
#   
...
parse_mx_record()
{
    ...
    info "    ⚙️  Running dig ${bluel}dig +short mx \"${fqdn}\"${greym}"
...
# #
#   Read stdin and parse SPF records to temp file
# #

if [ ! -f "${file_ipset_temp}" ]; then
    touch "${file_ipset_temp}"
fi
...
while IFS= read -r ip || [ -n "${ip}" ]; do
    [ -z "${ip}" ] && continue
    info "    📕 Running ${bluel}parse_spf_record${greym} on ip ${bluel}${ip}${greym}"
    parse_spf_record "${ip}" >> "${file_ipset_temp}"
done
...
info "    ✴️  Performing sed operations on ${bluel}${file_ipset_temp}${greym}"

# normalize CRLF
sed -i 's/\r$//' "${file_ipset_temp}"

# trim leading and trailing whitespace
sed -i 's/^[[:space:]]*//;s/[[:space:]]*$//' "${file_ipset_temp}"

# remove empty lines (after trimming)
sed -i '/^$/d' "${file_ipset_temp}"

# drop malformed entries before sorting (optional trusted-input fast path)
if [ "${argTrustedInput}" = "true" ]; then
    info "    ⚡ Trusted input mode enabled; skipping per-line IP validation"
else
    filter_valid_ip_entries "${file_ipset_temp}"
fi

# #
#   Dedupe, Sort: Move from .tmp to target
# #

info "    🔃 Sorting and deduplicating SPF results"
if [ "${argIncludeComments}" = "true" ]; then
    grep -vE '^[[:space:]]*(#|;|$)' "${file_ipset_temp}" | sort_results > "${file_ipset_target}"
else
    sort_results < "${file_ipset_temp}" > "${file_ipset_target}"
fi
rm -f "${file_ipset_temp}"
...
if ! has_valid_ip_entries "${file_ipset_target}"; then
    ...
    list_fallback_load "${file_ipset_target}" "${argFallbackBlock}" "1"
fi
