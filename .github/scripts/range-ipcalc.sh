...
#                           Uses linux package `ipcalc`; which may be resource intense on large lists.
#   @path               .github/scripts/range-ipcalc.sh
#   @params             .github/scripts/range-ipcalc.sh
#                           <argFileSource>     str         required
#                           <argFileSaveTo>     str         required
#   @commands           1.  .github/scripts/range-ipcalc.sh file_input.txt blocklists/file_output.ipset
#                       2.  .github/scripts/range-ipcalc.sh /dev/stdin output.txt <<EOF
#                               1.1.1.1 - 1.1.1.255
#                               8.8.8.0 - 8.8.8.255
#                           EOF
#                       3.  CFG_STDOUT=true .github/scripts/range-ipcalc.sh /dev/stdin output.txt <<EOF
#                               1.1.1.1 - 1.1.1.255
#                               8.8.8.0 - 8.8.8.255
#                           EOF
#   @structure          📁 .github
...
app_file_this=$(basename "$0")                                                  # range-ipcalc.sh   (with ext)
app_file_bin="${app_file_this%.*}"                                              # range-ipcalc      (without ext)
...
#   Blocklist › Main › Load
#   
#   @usage          list_main_load "${argFileSource}" "${file_ipset_target}" "$i"
#   @args           _fnArgSource        input local block file to fetch IPs from
...
list_main_load()
{
    ...
    info "    🌎 Converting IP ranges to blacklist in ${bluel}${PWD}/${_fnFileTemp}${greym}"
    ...
    if [ -t 0 ]; then
        ...
        # data is being piped in; use stdin
        exec < /dev/stdin
    fi
    ...
    unset   _fnArgUrl _fnArgFile _fnFileTemp _fnListNum \
...
i=1
list_main_load "${argFileSource}" "${file_ipset_target}" "$i"
